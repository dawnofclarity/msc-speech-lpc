...
fname = 'heed_f.wav'; sampleStart = 2019;
...
for lpcOrder = [4, 8, 12, 16, 20, 24, 28, 32, 36]

    %% take fft of signal time->signal domain
    fftResult = fft(arSample);

    fftPlot2 = abs(fftResult / sampleCount);    % Take magnitude to yield the double sampling plot
...
%     [lpcResult, lpcError] = arcov(arSample, lpcOrder);
    ...
    legendText = [legendText; sprintf('Order %d', lpcOrder)];
